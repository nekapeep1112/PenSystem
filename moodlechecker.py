"""
Moodle Custom Build Fingerprinting Agent
...
Что делает:
  ...
  - Claude анализирует всё и делает вывод о составе сборки

Установка:
  pip install requests beautifulsoup4 anthropic --break-system-packages

Запуск:
  python moodle_fingerprint.py https://your-moodle.com
"""
...
# ══════════════════════════════════════════════
# Claude: финальный анализ
# ══════════════════════════════════════════════

def analyze_with_claude(ctx: FingerprintResult) -> str:
    print("\n[Claude] Анализирую собранные данные...")
    ...
    response = client.messages.create(
        ...
        messages=[{"role": "user", "content": json.dumps(summary, ensure_ascii=False, indent=2)}]
    ).content[0].text
...
def main():
    if len(sys.argv) < 2:
        print("Использование: python moodle_fingerprint.py https://your-moodle.com")
        sys.exit(1)
    ...
    print("=" * 60)
    print("  Moodle Custom Build Fingerprinting Agent")
